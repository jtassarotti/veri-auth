...
Definition BoolTypedProph `{!heapGS_gen hlc Σ} := make_TypedProph BoolProph.
...
Definition IntTypedProph `{!heapGS_gen hlc Σ} := make_TypedProph IntProph.

Section string_proph.
  Definition string_from_val (v : val) : option string :=
    match v with
    | LitV(LitString s) => Some s
    | _                 => None
    end.

  Definition string_to_val (s : string) : val := LitV(LitString s).

  Lemma string_inj_prop : ∀ (s : string), string_from_val (string_to_val s) = Some s.
  Proof. done. Qed.

  Definition StringProph : TypedProphSpec :=
    mkTypedProphSpec string string_from_val string_to_val string_inj_prop.
End string_proph.

Definition StringTypedProph `{!heapGS_gen hlc Σ} := make_TypedProph StringProph.
