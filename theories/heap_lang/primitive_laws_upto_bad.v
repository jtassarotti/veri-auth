...
From auth.program_logic_upto_bad Require Export weakestpre total_weakestpre.
From auth.program_logic_upto_bad Require Import ectx_lifting total_ectx_lifting.
From auth.heap_lang Require Export class_instances.
From auth.heap_lang Require Import tactics notation.
From auth.base_logic Require Import hashes_auth.
...
Class heapGS_gen hlc Σ := HeapGS {
  ...
  #[global] heapGS_hashesGS :: gset_bijG Σ string string;
  heapGS_hashes_name : gname;
...
Definition hash_collision (σ : state) := has_collision σ.(hashed_strings).
...
Global Program Instance heapGS_irisGS `{!heapGS_gen hlc Σ} : irisGS_gen hlc heap_lang Σ := {
  ...
  state_interp σ step_cnt κs _ :=
    (gen_heap_interp σ.(heap) ∗ proph_map_interp κs σ.(used_proph_id) ∗
     steps_auth step_cnt ∗
     hashes_auth heapGS_hashes_name σ.(hashed_strings))%I;
  fork_post _ := True%I;
  bad_state := hash_collision;
...
Definition hashed `{!heapGS_gen hlc Σ} s := hashed heapGS_hashes_name s.
...
Proof.
  ...
  iIntros (σ1 ns κ κs m) "(Hσ & Hκ & Hsteps & Hhash)".
  iDestruct (steps_lb_valid with "Hsteps Hlb") as %?.
  iMod ("Hwp" $! σ1 ns κ κs m with "[$Hσ $Hκ $Hsteps $Hhash]") as "[%Hs Hwp]".
...
Proof.
  ...
  - iIntros (σ ns κ m) "(Hσ & Hκ & Hsteps & Hhi & Hhash)".
...
Proof.
  ...
  iIntros (σ1 ns κs nt) "(Hσ & Hκs & Hsteps & Hhash) !>".
  ...
  iMod (steps_auth_update_S with "Hsteps") as "Hsteps".
  iModIntro; do 2 (iSplit; first done). iFrame "Hσ Hκs Hsteps Hhash". iApply "HΦ".
...
Lemma twp_hash s E str :
  [[{ True }]]
    Hash (Val $ LitV $ LitString str) @ s; E
  [[{ RET LitV (LitString $ hash str); hashed str }]].
Proof.
  iIntros (Φ) "#Hi HΦ".
  iApply twp_lift_atomic_base_step_not_bad; [done|].
  iIntros (σ1 ns κs nt) "(?&?&Hsteps&Hhash) !>".
  iSplit; first by eauto with base_step.
  iIntros (κ v2 σ2 efs [Hstep Hbad]); inv_base_step.
  iMod (steps_auth_update_S with "Hsteps") as "Hsteps".
  iMod (hashes_auth_update with "Hhash") as "[%Hcoll | [H Hs]]".
  { iExFalso. iPureIntro.
    apply Hbad. apply collides_has_collision in Hcoll.
    rewrite /hash_collision /= union_comm_L //. }
  iSpecialize ("HΦ" with "Hs"). by iFrame.
Qed.

Lemma wp_hash s E str :
  {{{ True }}}
    Hash (Val $ LitV $ LitString str) @ s; E
  {{{ RET LitV (LitString $ hash str); hashed str }}}.
Proof.
  iIntros (Φ) "#Hi HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_hash with "[$]"). iIntros "Hs HΦ !>".
  by iApply "HΦ".
Qed.
...
Proof.
  ...
  iIntros (σ1 ns κ κs nt) "(Hσ & HR & Hsteps & Hhash) !>". iSplit; first by eauto with base_step.
  iIntros "!>" (v2 σ2 efs Hstep) "_". inv_base_step.
