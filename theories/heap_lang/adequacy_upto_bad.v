...
From auth.program_logic_upto_bad Require Export weakestpre adequacy.
From auth.heap_lang Require Import proofmode_upto_bad notation.
...
Definition heapΣ : gFunctors :=
  #[invΣ; gen_heapΣ loc (option val); inv_heapΣ loc (option val);
    proph_mapΣ proph_id (val * val); mono_natΣ; gset_bijΣ string string].
Global Instance subG_heapGpreS {Σ} : subG heapΣ Σ → heapGpreS Σ.
Proof. solve_inG. Qed.

Definition heap_adequacy_strong Σ `{!heapGpreS Σ} s e σ φ :
  (∀ `{!heapGS Σ}, ⊢ inv_heap_inv -∗
                     WP e @ s; ⊤ {{ v, ∀ σ n κ m, state_interp σ n κ m -∗ ⌜φ v σ⌝ }}) →
  adequate hash_collision s e σ φ.
Proof.
  intros Hwp.
  destruct (decide (hash_collision σ)) as [|Hnb].
  { by apply adequate_trivial. }
  apply adequate_alt; [done|]; intros t2 σ2 [n [κs Hn]]%erased_steps_nsteps_not_bad.
  ...
  iMod hash_auth_alloc as (γ') "Hhashes"; [done|].
  set (Hheap := (HeapGS _ _ _ _ _ _ _ γ' γ _)).
  iDestruct (Hwp Hheap with "Hi") as "Hwp".
  iModIntro.
  iExists (λ σ ns κs nt, (gen_heap_interp σ.(heap) ∗
                          proph_map_interp κs σ.(used_proph_id) ∗
                          mono_nat_auth_own γ 1 ns ∗
                          hashes_auth γ' σ.(hashed_strings)))%I.
...
Definition heap_adequacy Σ `{!heapGpreS Σ} s e σ φ :
  ...
  adequate hash_collision s e σ (λ v _, φ v).
Proof.
  intros Hwp.
...
Lemma heap_adequacy_strong_proph Σ `{!heapGpreS Σ} s (e : proph_id → expr) σ φ p :
  ...
  adequate hash_collision s (e p) σ φ.
Proof.
  intros Hi Hwp.
  destruct (decide (hash_collision σ)) as [|Hnb].
  { by apply adequate_trivial. }
