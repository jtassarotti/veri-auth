...
Section LinkedList.
  ...
  Fixpoint is_linked_list (l : loc) (vs : list A) : iProp Σ :=
    match vs with
    | nil => l G↦ NONEV
    | a :: vs => ∃ (l' : loc), l G↦ SOMEV ($a, #l') ∗ is_linked_list l' vs
  end.
  ...
  Lemma gwp_linked_list_empty E s :
    G{{{ True }}}
      empty #() @ s; E
    {{{ l, RET #l; is_linked_list l [] }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_linked_list_insert a l vs E s :
    G{{{ is_linked_list l vs }}}
      insert #l $a @ s; E
    {{{ l', RET #l'; is_linked_list l' (a :: vs) }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "H HΦ".
  ...
  Lemma gwp_linked_list_head l vs E s :
    G{{{ is_linked_list l vs }}}
      head #l @ s; E
    {{{ RET $(List.head vs); is_linked_list l vs }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_linked_list_tail l vs E s :
    G{{{ is_linked_list l vs }}}
      tail #l @ s; E
    {{{ (o : option val), RET $o;
        from_option
          (λ w, ∃ (l' : loc) a vs', ⌜vs = a :: vs'⌝ ∗ ⌜w = #l'⌝ ∗ l G↦ SOMEV ($a, #l') ∗ is_linked_list l' vs')
          (⌜vs = []⌝ ∗ is_linked_list l vs) o }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_linked_list_tail_cons E l vs a s :
    G{{{ is_linked_list l (a :: vs) }}}
      tail #l @ s; E
    {{{ (l' : loc), RET SOMEV #l'; l G↦ SOMEV ($a, #l') ∗ is_linked_list l' vs }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "H HΦ".
  ...
  Lemma gwp_linked_list_mem `{!EqDecision A} l (vs : list A) a E s :
    Forall (vals_compare_safe $a) (inject <$> vs) →
    G{{{ is_linked_list l vs }}}
      mem #l $a @ s ; E
    {{{ RET #(bool_decide (a ∈ vs)); is_linked_list l vs }}} ? gwp_laters g.
  Proof.
    iIntros (Hcmp Φ) "H HΦ".
    ...
    - rewrite {2}/mem /=.
      ...
      + iEval (rewrite bool_decide_eq_true_2).
        ...
        iApply "HΦ". by iFrame.
      + assert (bool_decide ($ a' = $ a) = false) as ->.
  ...
  #[local] Lemma gwp_linked_list_find_aux l vs (f : val) E s (n : nat) (P : A → Prop) `{∀ x, Decision (P x)} :
    (∀ a,
      G{{{ True }}} f $a @ s; E {{{ RET #(bool_decide (P a)); True }}} ? gwp_laters g) ⊢
    G{{{ is_linked_list l vs }}}
      find_aux #l f #n @ s ; E
    {{{ (o : option val), RET $o; is_linked_list l vs ∗
          from_option (λ w, ∃ (m : nat) a, ⌜w = (#(n + m), $a)%V⌝ ∗
                                ⌜vs !! m = Some a ∧ P a ∧ ∀ j y, vs !! j = Some y → j < m → ¬P y⌝)
          (⌜Forall (λ x, ¬ P x) vs⌝) o }}} ? gwp_laters g.
  Proof.
    iIntros "#Hf" (Ψ) "!# H HΨ".
  ...
  Lemma gwp_linked_list_find l vs (f : val) E s (P : A → Prop) `{∀ x, Decision (P x)} :
    (∀ a,
      G{{{ True }}} f $a @ s; E {{{ RET #(bool_decide (P a)); True }}} ? gwp_laters g) ⊢
    G{{{ is_linked_list l vs }}}
      find #l f @ s ; E
    {{{ RET $(list_find P vs); is_linked_list l vs }}} ? gwp_laters g.
  Proof.
    iIntros "#Hf" (Ψ) "!# H HΨ".
  ...
  Lemma gwp_linked_list_alter l vs idx (f : A → A) (fv : val) E s :
    (∀ a,
      G{{{ True }}} fv $a @ s; E {{{ RET $(f a); True }}} ? gwp_laters g) ⊢
    G{{{ is_linked_list l vs }}}
      alter #l fv #idx @ s ; E
    {{{ RET #(); is_linked_list l (list_alter f idx vs) }}} ? gwp_laters g.
  Proof.
    iIntros "#Hf" (Ψ) "!# Hl HΨ".
  ...
  Lemma gwp_linked_list_nth l vs (k : nat) E s:
    G{{{ is_linked_list l vs }}}
      nth #l #k @ s; E
    {{{ RET $(vs !! k); is_linked_list l vs }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "H HΦ".
  ...
  Lemma gwp_linked_list_filter l vs (f : val) E s (P : A → Prop) `{∀ x, Decision (P x)} :
    (∀ a,
      G{{{ True }}} f $a @ s; E {{{ RET #(bool_decide (P a)); True }}} ? gwp_laters g) ⊢
    G{{{ is_linked_list l vs }}}
      filter #l f @ s; E
     {{{ l', RET #l'; is_linked_list l' (base.filter P vs) }}} ? gwp_laters g.
  Proof.
    iIntros "#Hf" (Ψ) "!# Hl HΨ".
