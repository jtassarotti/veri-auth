...
From auth.heap_lang Require Export lang notation gen_weakestpre lib.inject.
...
Section list_specs.
  ...
  Lemma gwp_list_singleton E a s :
    G{{{ True }}}
      [$ a] @ s; E
    {{{ v, RET v; ⌜is_list [a] v⌝ }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_head E lv l s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_head lv @ s; E
    {{{ v, RET v;
        ⌜(l = [] ∧ v = NONEV) ∨ (∃ a l', l = a :: l' ∧ v = SOMEV (inject a))⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ a) "HΦ".
  ...
  Lemma gwp_list_head_cons E lv a l s :
    G{{{ ⌜is_list (a :: l) lv⌝ }}}
      list_head lv @ s; E
    {{{ RET (SOMEV (inject a)); True }}} ? gwp_laters g.
  Proof.
    iIntros (Φ Hl) "HΦ".
  ...
  Lemma gwp_list_head_opt E lv l s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_head lv @ s; E
    {{{ w, RET w; ⌜w = $(hd_error l)⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ a) "HΦ".
    gwp_lam. destruct l; simpl in *; subst.
  ...
  Lemma gwp_list_tail E lv l s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_tail lv @ s; E
    {{{ v, RET v; ⌜is_list (tail l) v⌝}}} ? gwp_laters g.
  Proof.
    iIntros (Φ a) "HΦ".
    gwp_lam. destruct l; simpl in *; subst.
  ...
  Lemma gwp_list_length E l lv s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_length lv @ s; E
    {{{ v, RET #v; ⌜v = length l⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "Ha HΦ".
  ...
  Lemma gwp_list_iter_invariant' Φ1 Φ2 (Ψ: list A → iProp _) P E l lv handler lrest s:
    (∀ (a : A) l',
        ...
        {{{v, RET v; P ∗ Ψ (l' ++ [a]) ∗ Φ2 (length l') a }}} ? gwp_laters g) -∗
    G{{{ ⌜is_list l lv⌝ ∗ P ∗ Ψ lrest ∗ [∗ list] i ↦ a ∈ l, Φ1 (length lrest + i) a}}}
      list_iter (Val handler) lv @ s; E
    {{{ RET #(); P ∗ Ψ (lrest++l) ∗ [∗ list] i ↦ a ∈ l, Φ2 (length lrest + i) a }}} ? gwp_laters g.
  Proof.
    rewrite /list_iter.
  ...
  Lemma gwp_list_iter_invariant Φ1 Φ2 (Ψ: list A -> iProp _) P E l lv handler s :
    ...
    G{{{ ⌜is_list l lv⌝ ∗ P ∗ Ψ [] ∗ [∗ list] i ↦ a ∈l, Φ1 i a}}}
      list_iter (Val handler) lv @ s; E
    {{{ RET #(); P ∗ Ψ l ∗ [∗ list] i ↦ a ∈ l, Φ2 i a}}} ? gwp_laters g.
  Proof.
  ...
  (*       G{{{ P ∗ Φ i a }}} *)
  (*         (Val handler) (Val (inject a)) @ s; E *)
  (*       {{{v, RET v; P ∗ Ψ i a }}} ? gwp_laters g) -∗ *)
  (*   G{{{ ⌜is_list l lv⌝ ∗ P ∗ [∗ list] i↦a ∈ l, Φ (n + i)%nat a }}} *)
  (*     list_iter (Val handler) lv @ s; E *)
  (*   {{{ RET #(); P ∗ [∗ list] i↦a ∈ l, Ψ (n + i)%nat a }}} ? gwp_laters g. *)
  (* Proof. *)
  (*   rewrite /list_iter. *)
  ...
  (*     list_iter (Val handler) lv @ s; E *)
  (*   {{{ RET #(); P ∗ [∗ list] i↦a ∈ l, Ψ i a }}} ? gwp_laters g. *)
  (* Proof. *)
  ...
  (*       G{{{ P ∗ Φ a }}} *)
  (*         (Val handler) (Val (inject a)) @ s; E *)
  (*       {{{v, RET v; P ∗ Ψ a }}} ? gwp_laters g) -∗ *)
  (*   G{{{ ⌜is_list l lv⌝ ∗ P ∗ [∗ list] a ∈ l, Φ a }}} *)
  (*     list_iter (Val handler) lv @ s; E *)
  (*   {{{ RET #(); P ∗ [∗ list] a ∈ l, Ψ a }}} ? gwp_laters g. *)
  (* Proof. *)
  (*   rewrite /list_iter. *)
  (*   iIntros "#H" (Φ') "!# (%Ha & HP & Hl) HΦ". *)
  ...
  Lemma gwp_list_iteri_loop
    ...
    is_list l lv →
    (∀ (i : nat) (x : A),
      G{{{ P ∗ Φ i x }}} fv #i (inject x) @ s; E {{{ v, RET v; P ∗ Ψ i x }}} ? gwp_laters g) -∗
    G{{{ P ∗ ([∗ list] i ↦ a ∈ l, Φ (k+i)%nat a) }}}
      list_iteri_loop fv #k lv @ s; E
    {{{ RET #(); P ∗ [∗ list] i ↦ a ∈ l, Ψ (k+i)%nat a }}} ? gwp_laters g.
  Proof.
    iIntros (Hl) "#Hf".
    iIntros (Φ') "!> [HP Hl] HΦ".
  ...
  Lemma gwp_list_fold P Φ Ψ E handler (l : list A) acc lv s :
    (∀ (a : A) acc lacc lrem,
        ...
        {{{v, RET v; P (lacc ++ [a]) v ∗ Ψ a }}} ? gwp_laters g) -∗
    G{{{ ⌜is_list l lv⌝ ∗ P [] acc ∗ [∗ list] a∈l, Φ a }}}
      list_fold handler acc lv @ s; E
    {{{v, RET v; P l v ∗ [∗ list] a∈l, Ψ a }}} ? gwp_laters g.
  Proof.
    ...
    □ (∀ (a : A) acc lacc lrem, (P lacc acc None (a::lrem) -∗ P lacc acc (Some a) lrem))%I -∗
      (∀ (a : A) acc lacc lrem,
          G{{{ ⌜lp ++ l = lacc ++ a :: lrem⌝ ∗ P lacc acc (Some a) lrem ∗ Φ a }}}
            (Val handler) (Val acc) (inject a) @ s; E
          {{{v, RET v; P (lacc ++ [a]) v None lrem ∗ Ψ a }}} ? gwp_laters g) -∗
    G{{{ ⌜is_list l lv⌝ ∗ P lp acc None l ∗ [∗ list] a∈l, Φ a }}}
      list_fold (Val handler) (Val acc) (Val lv) @ s; E
    {{{v, RET v; P (lp ++ l) v None [] ∗ [∗ list] a∈l, Ψ a }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_fold' E handler (l : list A) acc lv P Φ Ψ s :
    ...
    {{{v, RET v; P l v None [] ∗ [∗ list] a∈l, Ψ a }}} ? gwp_laters g.
  Proof.
    iIntros "#Hvs #Hcl".
  ...
  Lemma gwp_list_sub E k l lv  s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_sub #k lv @ s; E
    {{{ v, RET v; ⌜is_list (take k l) v ⌝}}} ? gwp_laters g.
  Proof.
   iIntros (Φ) "Hcoh HΦ".
  ...
  Lemma gwp_list_nth E (i: nat) l lv s :
    ...
    {{{ v, RET v; (⌜v = NONEV⌝ ∧ ⌜length l <= i⌝) ∨
              ⌜∃ r, v = SOMEV (inject r) ∧ nth_error l i = Some r⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "Ha HΦ".
  ...
  Lemma gwp_list_nth_some E (i: nat) l lv  s :
    G{{{ ⌜is_list l lv ∧ i < length l⌝ }}}
      list_nth (Val lv) #i @ s; E
    {{{ v, RET v; ⌜∃ r, v = SOMEV (inject r) ∧ nth_error l i = Some r⌝ }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_mem (l : list A) lv a s E :
    ...
    G{{{ ⌜is_list l lv⌝ }}}
      list_mem ($a) lv @ s; E
    {{{ (b : bool), RET #b; if b then ⌜a ∈ l⌝ else ⌜a ∉ l ∨ l = nil⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (? Φ).
    iInduction l as [|a' l'] "IH" forall (lv Φ);
  ...
  Lemma gwp_find_remove E (l : list A) lv (Ψ : A → iProp _) (fv : val) s :
    (∀ (a : A),
        G{{{ True }}}
          fv (inject a) @ s; E
        {{{ (b : bool), RET #b; if b then Ψ a else True }}} ? gwp_laters g) -∗
    G{{{ ⌜is_list l lv⌝ }}}
  ...
  Local Lemma gwp_list_rev_aux E l lM r rM s :
   ...
   {{{ v, RET v; ⌜is_list (rev_append lM rM) v⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (? [Hl Hr]) "H".
 ...
 Qed.
  ...
  Lemma gwp_list_rev E l lM s :
    G{{{ ⌜is_list lM l⌝ }}}
      list_rev (Val l) @ s; E
    {{{ v, RET v; ⌜is_list (reverse lM) v⌝ }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_append E l lM r rM s :
    ...
    {{{ v, RET v; ⌜is_list (lM ++ rM) v⌝ }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_forall Φ Ψ E (l : list A) lv (fv : val) s :
    (∀ (a : A),
        G{{{ True }}}
          fv (inject a) @ s; E
        {{{ (b : bool), RET #b; if b then Φ a else Ψ a }}} ? gwp_laters g) -∗
    G{{{ ⌜is_list l lv⌝ }}}
      list_forall fv lv @ s; E
    {{{ (b : bool), RET #b; if b then [∗ list] a ∈ l, Φ a else ∃ a, ⌜a ∈ l⌝ ∗ Ψ a }}} ? gwp_laters g.
  Proof.
    rewrite /list_forall.
  ...
  Lemma gwp_list_is_empty l v E s :
    G{{{ ⌜is_list l v⌝ }}}
      list_is_empty v @ s; E
    {{{ RET #(match l with [] => true | _ => false end); True }}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_filter (l : list A) (P : A -> bool) (f lv : val) E s :
    G{{{ (∀ (x : A),
            G{{{ True }}}
              f (inject x) @ s; E
            {{{ w, RET w; ⌜w = inject (P x)⌝ }}} ? gwp_laters g) ∗
        ⌜is_list l lv⌝ }}}
       list_filter f lv @ s; E
     {{{ rv, RET rv; ⌜is_list (List.filter P l) rv⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "[#Hf %Hil] HΦ".
...
Section list_specs_extra.
  ...
  Lemma gwp_list_map `{!Inject B val} (l : list A) (f : A -> B) (fv lv : val) E s :
    G{{{ (∀ (x : A),
          G{{{ True }}}
            fv (inject x) @ s; E
          {{{ fr, RET fr; ⌜fr = inject (f x)⌝ }}} ? gwp_laters g) ∗
          ⌜is_list l lv⌝ }}}
      list_map fv lv @ s; E
    {{{ rv, RET rv; ⌜is_list (List.map f l) rv⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "[#Hf %Hil] HΦ".
  ...
  Lemma gwp_list_mapi_loop `{!Inject B val}
    ...
    G{{{ □ (∀ (i : nat) (x : A),
              G{{{ γ (k + i)%nat x }}}
                ...
                {{{ fr, RET fr;
                    ...
                    ⌜fr = (inject r)⌝ ∗ ψ (k + i)%nat r }}} ? gwp_laters g) ∗
        ⌜is_list l lv⌝ ∗
    ...
    {{{ rv, RET rv;
        ...
        ⌜is_list l' rv⌝ ∗
        ([∗ list] i ↦ a ∈ l', ψ (k + i)%nat a)}}} ? gwp_laters g.
  Proof.
  ...
  Lemma gwp_list_mapi `{!Inject B val}
    ...
    G{{{ □ (∀ (i : nat) (x : A),
              G{{{ γ i x }}}
                ...
                {{{ fr, RET fr;
                    let r := f i x in
                    ⌜fr = (inject r)⌝ ∗ ψ i r }}} ? gwp_laters g) ∗
        ⌜is_list l lv⌝ ∗
        ([∗ list] i ↦ a ∈ l, γ i a)
    }}}
      list_mapi fv lv @ s; E
  ...
  Lemma gwp_list_seq E n m s :
    G{{{ True }}}
      list_seq #n #m @ s; E
    {{{ v, RET v; ⌜is_list (seq n m) v⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "_ Hφ".
  ...
  Lemma gwp_list_suf E (n:nat) l lv s :
    G{{{ ⌜is_list l lv⌝ }}}
      list_suf #n lv @ s; E
    {{{ v, RET v; ⌜is_list (drop n l) v⌝ }}} ? gwp_laters g.
  Proof.
   iIntros (Φ) "%Hl HΦ".
  ...
  Lemma gwp_list_inf_ofs E (n ofs:nat) l lv s :
    ...
    {{{ v, RET v; ⌜is_list (take ofs (drop n l)) v⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "%Hl HΦ".
  ...
  Lemma gwp_list_inf E (i j:nat) l lv s :
    ...
    G{{{ ⌜is_list l lv⌝ }}}
      list_inf #i #j lv @ s; E
    {{{ v, RET v; ⌜is_list (take (j-i+1) (drop i l)) v⌝ }}} ? gwp_laters g.
  Proof.
