...
Section set_specs.
  ...
  Definition is_set (X : gset A) (v : val) : Prop :=
    ∃ (l : list A), is_list l v ∧ X = list_to_set l ∧ NoDup l ∧ set_Forall (λ a, val_is_unboxed $a) X .

  Lemma gwp_set_empty s E :
    G{{{ True }}}
      set_empty #() @ s; E
    {{{ v, RET v; ⌜is_set ∅ v⌝}}} ? gwp_laters g.
  Proof.
    iIntros (Φ) "_ HΦ".
    gwp_rec. gwp_pures.
    iModIntro. iApply "HΦ".
  ...
  Lemma gwp_set_add x X v s E :
    ...
    G{{{ ⌜is_set X v⌝ }}}
      set_add $x v @ s; E
    {{{ v, RET v; ⌜is_set ({[ x ]} ∪ X) v⌝}}} ? gwp_laters g.
  Proof.
  ...
  (* TODO: [is_list] should just remember that the contents of the list is unboxed so [gwp_list_mem]
     works without the [val_is_unboxed] assumption *)
  Lemma gwp_set_mem x X v s E :
    val_is_unboxed $ x →
    G{{{ ⌜is_set X v⌝ }}}
      set_mem $x v @ s; E
    {{{ (b : bool), RET #b; ⌜if b then x ∈ X else x ∉ X⌝ }}} ? gwp_laters g.
  Proof.
    iIntros (? Φ (l & ? & -> & ?)) "HΦ".
  ...
  (*      G{{{ ⌜x ∈ X⌝ ∗ P ∗ Ψ X' ∗ Φ1 x }}} *)
  (*        (Val handler) $x @ s; E *)
  (*      {{{v, RET v; P ∗ Ψ (X' ∪ {[x]}) ∗ Φ2 x }}} ? gwp_laters g) -∗ *)
  (*   G{{{ ⌜is_set X v⌝ ∗ P ∗ Ψ ∅ ∗ [∗ set] x ∈ X, Φ1 x }}} *)
  (*     set_iter (Val handler) (Val v) @ s; E *)
  (*   {{{ RET #(); P ∗ Ψ X ∗ [∗ set] x ∈ X, Φ2 x }}} ? gwp_laters g. *)
  (* Proof. *)
  (*   iIntros "#Hhandler". *)
  ...
  (*   {{{ (b : bool), RET #b; ⌜if b then X ⊆ Y else X ⊈ Y⌝ }}} ? gwp_laters g. *)
  (* Proof. *)
  (*   iIntros (Φ) "[%HX %HY] HΦ". *)
  (*   destruct HX as (Xl & ? & -> & ? & ?). *)
