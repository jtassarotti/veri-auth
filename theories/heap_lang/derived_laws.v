...
From auth.heap_lang Require Export primitive_laws.
...
#[local] Lemma gwp_mixin `{!heapGS_gen hlc Σ} :
  GenWpMixin true wp (λ l dq v, l ↦{dq} v)%I.
Proof.
  constructor; intros.
  - apply _.
  - apply pointsto_agree.
  - rewrite !uPred.discrete_valid. apply pointsto_valid.
  - iIntros "H1 H2".
    by iPoseProof (pointsto_valid_2 with "H1 H2") as "(% & %)".
  - by apply wp_value.
  - apply wp_fupd.
  - apply wp_bind. apply _.
  - iIntros "?". iApply lifting.wp_pure_step_later; [done|]. by iIntros "!> _".
  - iIntros "H". iApply wp_alloc; [done|]. iIntros (?) "!> [? ?]". by iApply "H".
  - apply wp_load.
  - apply wp_store.
Qed.

Canonical Structure gwp_wpre `{!heapGS_gen hlc Σ} := Build_GenWp gwp_mixin.
