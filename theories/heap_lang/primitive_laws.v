...
From auth.heap_lang Require Import tactics notation gen_weakestpre.
From auth.base_logic Require Export hashes_auth.
...
Lemma twp_hash s E str :
  ...
  [[{ RET LitV (LitString $ hash str); True }]].
Proof.
  iIntros (Φ) "#Hi HΦ".
  iApply twp_lift_atomic_base_step; [done|].
  iIntros (σ1 ns κs nt) "(?&?&Hsteps) !>".
  iSplit; first by eauto with base_step.
  iIntros (κ v2 σ2 efs Hstep); inv_base_step.
  iMod (steps_auth_update_S with "Hsteps") as "Hsteps".
  (* iMod (hashes_auth_update with "Hhash") as "[$ Hs]". *)
  iSpecialize ("HΦ" with "[//]"). by iFrame.
Qed.
