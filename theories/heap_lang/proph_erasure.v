...
Fixpoint erase_expr (e : expr) : expr :=
  ...
  | TernOp op e1 e2 e3 => TernOp op (erase_expr e1) (erase_expr e2) (erase_expr e3)
...
Fixpoint erase_ectx_item (Ki : ectx_item) : list ectx_item :=
  ...
  | TernOp1Ctx op v2 v3 => [TernOp1Ctx op (erase_val v2) (erase_val v3)]
  | TernOp2Ctx op e1 v3 => [TernOp2Ctx op (erase_expr e1) (erase_val v3)]
  | TernOp3Ctx op e1 e2 => [TernOp3Ctx op (erase_expr e1) (erase_expr e2)]
...
Definition erase_state (σ : state) : state :=
  {| heap := erase_heap (heap σ); used_proph_id := ∅;
    hashed_strings := σ.(hashed_strings) |}.
...
Proof.
  rewrite /bin_op_eval /bin_op_eval_int /bin_op_eval_bool
    /bin_op_eval_loc /bin_op_eval_string;
    split; [intros ?|intros (?&?&?)];
...
Lemma tern_op_eval_erase op v1 v2 v3 v' :
  tern_op_eval op (erase_val v1) (erase_val v2) (erase_val v3) = Some v' ↔
  ∃ w, tern_op_eval op v1 v2 v3 = Some w ∧ erase_val w = v'.
Proof.
  rewrite /tern_op_eval /option_nat_to_val;
    split; [intros ?|intros(?&?&?)];
    repeat (case_match; simplify_eq/=); eauto.
Qed.
  ...
  base_steps_to_erasure_of
    ...
    (if bool_decide (vl = erase_val v)
     then {| heap := <[l:=Some $ erase_val w]> (erase_heap (heap σ));
             used_proph_id := ∅; hashed_strings := σ.(hashed_strings) |}
     else erase_state σ) [].
Proof.
  intros Hl Hvl.
  rewrite lookup_erase_heap in Hl.
...
Lemma erased_base_step_base_step_FAA l n m σ :
  ...
  base_steps_to_erasure_of
    (FAA #l #m) σ #n
    {| heap := <[l:= Some #(n + m)]> (erase_heap (heap σ)); used_proph_id := ∅;
      hashed_strings := σ.(hashed_strings) |} [].
Proof.
  intros Hl.
  rewrite lookup_erase_heap in Hl.
...
Proof.
  ...
  inversion Hhstep; simplify_eq/=;
    repeat match goal with
           ...
           | H : tern_op_eval _ (erase_val _) (erase_val _) (erase_val _) = Some _ |- _ =>
             apply tern_op_eval_erase in H as [? [? ?]]
...
Lemma base_step_erased_prim_step_tern_op σ op v1 v2 v3 v':
  tern_op_eval op v1 v2 v3 = Some v' →
  ∃ e2' σ2' ef', prim_step (TernOp op (erase_val v1) (erase_val v2) (erase_val v3))
                           (erase_state σ) [] e2' σ2' ef'.
Proof.
  do 3 eexists; apply base_prim_step; econstructor.
  apply tern_op_eval_erase; eauto.
Qed.
...
Proof.
  induction 1; simplify_eq/=;
    eauto using base_step_erased_prim_step_CmpXchg,
                ...
                base_step_erased_prim_step_tern_op,
