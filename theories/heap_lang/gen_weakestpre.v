...
Section GenWp_mixin.
  Context {Σ : gFunctors} {A : Type} `{!invGS_gen hlc Σ}.
  Context (laters : bool).
  Context (gwp : A → coPset → expr  → (val → iProp Σ) → iProp Σ).
  ...
  Record GenWpMixin := {
    ...
    mixin_gwp_bind K s E e Φ :
      gwp s E e (λ v, gwp s E (fill K (of_val v)) Φ ) ⊢ gwp s E (fill K e) Φ;
    mixin_gwp_pure_step E e1 e2 φ n Φ s :
      PureExec φ n e1 e2 →
      φ →
      ▷^(if laters then n else 0) gwp s E e2 Φ ⊢ gwp s E e1 Φ;

    mixin_gwp_alloc E (v : val) s Φ :
     ▷?laters (∀ l, gwp_pointsto l (DfracOwn 1) v -∗ Φ #l) ⊢ gwp s E (ref v) Φ;
    mixin_gwp_load E v l dq s Φ :
      ▷?laters gwp_pointsto l dq v -∗
      ▷?laters (gwp_pointsto l dq v -∗ Φ v) -∗
      gwp s E (! #l) Φ;
    mixin_gwp_store E (v v' : val) l s Φ :
...
Structure GenWp Σ `{invGS_gen hlc Σ} := {
  ...
  gwp : gwp_type → coPset → expr  → (val → iProp Σ) → iProp Σ;
...
Arguments gwp_type {_ _ _}.
Arguments gwp_laters {_ _ _}.


Notation "'GWP' e @ s ; E {{ Φ } }" := (gwp s E e%E Φ)
  (at level 20, e, Φ at level 200, only parsing) : bi_scope.
Notation "'GWP' e @ s ; E {{ v , Q } }" := (gwp s E e%E (λ v, Q))
  (at level 20, e, Q at level 200,
   format "'[hv' 'GWP'  e  '/' @  '[' s ;  '/' E  ']' '/' {{  '[' v ,  '/' Q  ']' } } ']'") : bi_scope.

(* Texan triples *)
Notation "'G{{{' P } } } e @ s ; E {{{ x .. y , 'RET' pat ; Q } } } ? b" :=
  (□ ∀ Φ,
      P -∗ ▷?b (∀ x, .. (∀ y, Q -∗ Φ pat%V) .. ) -∗ GWP e @ s; E {{ Φ }})%I
    (at level 20, x closed binder, y closed binder,
     format "'[hv' G{{{  '[' P  ']' } } }  '/  ' e  '/' @  '[' s ;  '/' E  ']' '/' {{{  '[' x  ..  y ,  RET  pat ;  '/' Q  ']' } } }  ?  b ']'") : bi_scope.

Notation "'G{{{' P } } } e @ s ; E {{{ 'RET' pat ; Q } } } ? b" :=
  (□ ∀ Φ, P -∗ ▷?b (Q -∗ Φ pat%V) -∗ GWP e @ s; E {{ Φ }})%I
    (at level 20,
     format "'[hv' G{{{  '[' P  ']' } } }  '/  ' e  '/' @  '[' s ;  '/' E  ']' '/' {{{  '[' RET  pat ;  '/' Q  ']' } } }  ?  b ']'") : bi_scope.

(** Aliases for stdpp scope -- they inherit the levels and format from above. *)
Notation "'G{{{' P  } } } e @ s ; E {{{ x .. y , 'RET' pat ; Q  } } }  ?  b" :=
  (∀ Φ, P -∗ ▷?b (∀ x, .. (∀ y, Q -∗ Φ pat%V) .. ) -∗ GWP e @ s; E {{ Φ }}) : stdpp_scope.
Notation "'G{{{' P  } } } e @ s ; E {{{ 'RET' pat ; Q  } } }  ?  b " :=
  (∀ Φ, P -∗ ▷?b (Q -∗ Φ pat%V) -∗ GWP e @ s; E {{ Φ }}) : stdpp_scope.
...
Section genWp.
  ...
  Proof. apply gwp_mixin. Qed.
  Lemma gwp_fupd E Φ e s : GWP e @ s ; E {{ v, |={E}=> Φ v }} ⊢ GWP e @ s ; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.
  Lemma gwp_bind K s E e Φ :
    GWP e @ s ; E {{ v, GWP fill K (of_val v) @ s ; E {{ Φ  }} }} ⊢ GWP fill K e @ s ; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.
  Lemma gwp_pure_step E e1 e2 φ n Φ s :
    PureExec φ n e1 e2 →
    φ →
    ▷^(if gwp_laters g then n else 0) GWP e2 @ s ; E {{ Φ }} ⊢ GWP e1 @ s ; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.

  Notation "l G↦ dq v" := (gwp_pointsto g l dq v)
    (at level 20, dq custom dfrac at level 1, format "l  G↦ dq  v") : bi_scope.

  Lemma gwp_alloc E (v : val) s Φ :
    ▷?(gwp_laters g) (∀ l, l G↦ v -∗ Φ #l) ⊢ GWP ref v @ s; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.
  Lemma gwp_load E l v s (dq : dfrac) Φ :
    ▷?(gwp_laters g) l G↦{dq} v -∗ ▷?(gwp_laters g) (l G↦{dq} v -∗ Φ v) -∗ GWP (! #l) @ s; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.
  Lemma gwp_store E l (v v' : val) s Φ:
    ▷?(gwp_laters g) l G↦ v' -∗ ▷?(gwp_laters g) (l G↦ v -∗ Φ #()) -∗ GWP #l <- v @ s; E {{ Φ }}.
  Proof. apply gwp_mixin. Qed.

  Lemma tac_gwp_expr_eval Δ s E Φ e e' :
    (∀ (e'':=e'), e = e'') →
    envs_entails Δ (GWP e' @ s ; E {{ Φ }}) → envs_entails Δ (GWP e @ s ; E {{ Φ }}).
  Proof. by intros ->. Qed.

  Lemma tac_gwp_pure_later Δ Δ' E K e1 e2 φ n Φ s :
    PureExec φ n e1 e2 →
    φ →
    MaybeIntoLaterNEnvs (if gwp_laters g then n else 0) Δ Δ' →
    envs_entails Δ' (GWP (fill K e2) @ s ; E {{ Φ }}) →
    envs_entails Δ (GWP (fill K e1) @ s ; E {{ Φ }}).
  Proof.
  ...
  Lemma tac_gwp_value_nofupd Δ E Φ v s :
    envs_entails Δ (Φ v) → envs_entails Δ (GWP (of_val v) @ s ; E {{ Φ }}).
  Proof. rewrite envs_entails_unseal=> ->. apply gwp_value. Qed.

  Lemma tac_gwp_value' Δ E Φ v s :
    envs_entails Δ (|={E}=> Φ v) → envs_entails Δ (GWP (of_val v) @ s ; E {{ Φ }}).
  Proof. rewrite envs_entails_unseal=> ->. by rewrite -gwp_fupd -gwp_value. Qed.

  Lemma tac_gwp_bind K Δ E Φ e f s :
    f = (λ e, fill K e) → (* as an eta expanded hypothesis so that we can `simpl` it *)
    envs_entails Δ (GWP e @ s ; E {{ v, GWP f (Val v) @ s ; E {{ Φ }} }})%I →
    envs_entails Δ (GWP fill K e @ s ; E {{ Φ }}).
...
Tactic Notation "gwp_expr_eval" tactic3(t) :=
  iStartProof;
  lazymatch goal with
  | |- envs_entails _ (gwp ?s ?E ?e ?Q) =>
    notypeclasses refine (tac_gwp_expr_eval _ _ _ _ e _ _ _);
...
Ltac gwp_value_head :=
  lazymatch goal with
  | |- envs_entails _ (gwp ?s ?E (of_val _) (λ _, fupd ?E _ _)) =>
      eapply tac_gwp_value_nofupd
  | |- envs_entails _ (gwp ?s ?E (of_val _) (λ _, wp _ ?E _ _)) =>
      eapply tac_gwp_value_nofupd
  | |- envs_entails _ (gwp ?s ?E (of_val _) _) =>
...
Section gwp_heap.
  ...
  Lemma tac_gwp_alloc Δ Δ' E j K v Φ s :
    MaybeIntoLaterNEnvs (if gwp_laters g then 1 else 0) Δ Δ' →
    (∀ (l : loc),
        match envs_app false (Esnoc Enil j (l G↦ v)) Δ' with
        | Some Δ'' =>
            envs_entails Δ'' (GWP fill K (Val #l) @ s ; E {{ Φ }})
        | None => False
        end) →
    envs_entails Δ (GWP fill K (ref v) @ s ; E {{ Φ }}).
  Proof.
  ...
  Lemma tac_gwp_load Δ Δ' E i K b l dq v Φ s :
    MaybeIntoLaterNEnvs (if gwp_laters g then 1 else 0) Δ Δ' →
    envs_lookup i Δ' = Some (b, (l G↦ {dq} v)%I) →
    envs_entails Δ' (GWP fill K (Val v) @ s ; E {{ Φ }}) →
    envs_entails Δ (GWP fill K (! #l) @ s ; E {{ Φ }}).
  Proof.
  ...
  Lemma tac_gwp_store Δ Δ' E i K l v v' Φ s :
    MaybeIntoLaterNEnvs (if gwp_laters g then 1 else 0) Δ Δ' →
    envs_lookup i Δ' = Some (false, l G↦ v)%I →
    match envs_simple_replace i false (Esnoc Enil i (l G↦ v')) Δ' with
    | Some Δ'' => envs_entails Δ'' (GWP fill K (Val #()) @ s ; E {{ Φ }})
    | None => False
    end →
    envs_entails Δ (GWP fill K (#l <- v') @ s ; E {{ Φ }}).
  Proof.
...
Tactic Notation "gwp_load" :=
  let solve_gwp_pointsto _ :=
    ...
    iAssumptionCore || fail "gwp_load: cannot find" l "G↦ ?" in
  gwp_pures;
  lazymatch goal with
  | |- envs_entails _ (gwp ?s ?E ?e ?Q) =>
      first
...
Tactic Notation "gwp_store" :=
  let solve_gwp_pointsto _ :=
    let l := match goal with |- _ = Some (_, (gwp_pointsto _ ?l _ _)%I) => l end in
    iAssumptionCore || fail "gwp_store: cannot find" l "G↦ ?" in
  gwp_pures;
  lazymatch goal with
  | |- envs_entails _ (gwp ?s ?E ?e ?Q) =>
      first
