...
From auth.rel_logic_tern Require Export compatibility interp.
...
Section fundamental.
  ...
  Local Ltac intro_clause' := intro_clause; iIntros (????) "Hp Hi".
  Local Ltac pures :=
  ...
  Lemma bin_log_related_pair Θ Δ Γ e1 e2 e1' e2' e1'' e2'' τ1 τ2 :
  ...
  Lemma bin_log_related_fst Θ Δ Γ e e' e'' τ1 τ2 :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ1 * τ2) -∗
    {Θ;Δ;Γ} ⊨ Fst e ≤log≤ Fst e' ≤log≤ Fst e'' : τ1.
  Proof.
    iIntros "IH".
    intro_clause'.
    rel_bind_ap e e' e'' "IH" v w u "(Hp & Hi & H)".
    iDestruct "H" as (v1 v2 w1 w2) "(% & % & % & % & % & IHw & IHw')". simplify_eq/=.
    pures. by iFrame.
  Qed.

  Lemma bin_log_related_snd Θ Δ Γ e e' e'' τ1 τ2 :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'': τ1 * τ2) -∗
  ...
  Lemma bin_log_related_app Θ Δ Γ e1 e2 e1' e2' e1'' e2'' τ1 τ2 :
  ...
  Lemma bin_log_related_rec Θ Δ (Γ : stringmap (typ ⋆ Θ)) (f x : binder) (e e' e'' : expr) τ1 τ2 :
  ...
  Lemma bin_log_related_tapp' Θ Δ κ Γ e e' e'' τ τ' :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : ∀: κ, τ) -∗
    {Θ;Δ;Γ} ⊨ e #~ ≤log≤ e' #~ ≤log≤ e'' #~ : τ.[τ'/].
  Proof.
    iIntros "IH".
    intro_clause'.
    rel_bind_ap e e' e'' "IH" v v' v'' "(Hp & Hi & IH)"; fold kindO.
  ...
  Lemma bin_log_related_tapp Θ Δ κ (A : kindO Σ κ) Γ e e' e'' τ :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'': ∀: κ, τ) -∗
    {Θ ▹ κ; ext Δ A; ⤉Γ} ⊨ e #~ ≤log≤ e' #~ ≤log≤ e'' #~ : τ.
  Proof.
    ...
    rewrite -shift_env_eq /=.
    rel_bind_ap e e' e'' "IH" v v' v'' "(Hp & Hi & IH)"; fold kindO.
    wp_apply ("IH" $! _ #~ #~ #~ with "[//] Hp Hi").
  Qed.

  Lemma bin_log_related_seq Θ κ (A : kindO Σ κ) τ1 τ2 Δ Γ e1 e2 e1' e2' e1'' e2'' :
  ...
  Lemma bin_log_related_seq' Θ Δ Γ e1 e2 e1' e2' e1'' e2'' τ1 τ2 :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : τ1) -∗
    ({Θ;Δ;Γ} ⊨ e2 ≤log≤ e2' ≤log≤ e2'' : τ2) -∗
    {Θ;Δ;Γ} ⊨ (e1;; e2) ≤log≤ (e1';; e2') ≤log≤ (e1'';; e2'') : τ2.
  Proof.
  ...
  Lemma bin_log_related_injl Θ Δ Γ e e' e'' τ1 τ2 :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ1) -∗
    {Θ;Δ;Γ} ⊨ InjL e ≤log≤ InjL e' ≤log≤ InjL e'' : τ1 + τ2.
  Proof.
    iIntros "IH".
    intro_clause'.
    rel_bind_ap e e' e'' "IH" v v' v'' "(? & ? & H)".
    pures. iFrame. iModIntro.
    iExists _, _, _. eauto.
  Qed.

  Lemma bin_log_related_injr Θ Δ Γ e e' e'' τ1 τ2 :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ2) -∗
    {Θ;Δ;Γ} ⊨ InjR e ≤log≤ InjR e' ≤log≤ InjR e'' : τ1 + τ2.
  Proof.
    iIntros "IH".
    intro_clause'.
    rel_bind_ap e e' e'' "IH" v v' v'' "(? & ? & Hvv)".
    pures. iFrame. iModIntro.
    iExists _, _, _. eauto.
  Qed.

  Lemma bin_log_related_case Θ Δ Γ e0 e1 e2 e0' e1' e2' e0'' e1'' e2'' τ1 τ2 τ3 :
  ...
  Proof.
    iIntros "IH1 IH2 IH3".
    intro_clause'.
    rel_bind_ap e0 e0' e0'' "IH1" v0 v0' v'' "(Hp & Hi & IH1)".
    iDestruct "IH1" as (w w' w'') "[(% & % & % & #Hw) | (% & % & % & #Hw)]";
      simplify_eq/=; pures.
    - iApply (bin_log_related_app _ Δ Γ _ w _ w' _ w'' with "IH2 [] Hvs Hp Hi").
      iIntros (?) "?". iIntros (????) "Hp Hi".
      wp_pures. by iFrame.
    - iApply (bin_log_related_app _ Δ Γ _ w _ w' _ w'' with "IH3 [] Hvs Hp Hi").
      iIntros (?) "?"; iIntros (????) "Hp Hi".
      wp_pures. by iFrame.
  Qed.

  Lemma bin_log_related_if Θ Δ Γ e0 e1 e2 e0' e1' e2' e0'' e1'' e2'' τ :
  ...
  Proof.
    iIntros "IH1 IH2 IH3".
    intro_clause'.
    rel_bind_ap e0 e0' e0'' "IH1" v0 v0' v0'' "(Hp & Hi & IH1)".
  ...
  Lemma bin_log_related_store Θ Δ Γ e1 e2 e1' e2' e1'' e2'' τ :
  ...
  Lemma bin_log_related_xchg Θ Δ Γ e1 e2 e1' e2' e1'' e2'' τ :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : ref τ) -∗
    ({Θ;Δ;Γ} ⊨ e2 ≤log≤ e2' ≤log≤ e2'' : τ) -∗
  ...
  Lemma bin_log_related_FAA Θ Δ Γ e1 e2 e1' e2' e1'' e2'' :
  ...
  Lemma bin_log_related_CmpXchg_EqType Θ Δ Γ e1 e2 e3 e1' e2' e3' e1'' e2'' e3'' τ
    (HEqτ : EqType τ)
    (HUbτ : UnboxedType τ) :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : ref τ) -∗
    ({Θ;Δ;Γ} ⊨ e2 ≤log≤ e2' ≤log≤ e2'' : τ) -∗
  ...
  Lemma bin_log_related_CmpXchg Θ Δ Γ e1 e2 e3 e1' e2' e3' e1'' e2'' e3'' τ
    (HUbτ : UnboxedType τ) :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : ref τ) -∗
    ({Θ;Δ;Γ} ⊨ e2 ≤log≤ e2' ≤log≤ e2'' : τ) -∗
    ({Θ;Δ;Γ} ⊨ e3 ≤log≤ e3' ≤log≤ e3'' : τ) -∗
  ...
  Lemma bin_log_related_alloc Θ Δ Γ e e' e'' τ :
  ...
  Lemma bin_log_related_nat_binop Θ Δ Γ op e1 e2 e1' e2' e1'' e2'' τ :
  ...
  Lemma bin_log_related_bool_binop Θ Δ Γ op e1 e2 e1' e2' e1'' e2'' τ :
  ...
  Proof.
    iIntros (Hopτ) "IH1 IH2".
    intro_clause'.
    rel_bind_ap e2 e2' e2'' "IH2" v2 v2' v3'' "(?&?&IH2)".
    rel_bind_ap e1 e1' e1'' "IH1" v1 v1' v1'' "(?&?&IH1)".
    iDestruct "IH1" as (n) "(% & % & %)"; simplify_eq/=.
    iDestruct "IH2" as (n') "(% & % & %)"; simplify_eq/=.
  ...
  Lemma bin_log_related_strindex Θ Δ Γ e1 e2 e3 e1' e2' e3' e1'' e2'' e3'' :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : t_nat) -∗
  ...
  Proof.
    ...
    intro_clause'.
    rel_bind_ap e3 e3' e3'' "IH3" v3 v3' v3'' "(?&?&IH3)".
    rel_bind_ap e2 e2' e2'' "IH2" w2 w2' w3'' "(?&?&IH2)".
    rel_bind_ap e1 e1' e1'' "IH1" u1 u1' u1'' "(?&?&IH1)".
    iDestruct "IH1" as (n) "(% & % & %)"; simplify_eq/=.
  ...
  Lemma bin_log_related_strsub Θ Δ Γ e1 e2 e3 e1' e2' e3' e1'' e2'' e3'' :
    ({Θ;Δ;Γ} ⊨ e1 ≤log≤ e1' ≤log≤ e1'' : t_nat) -∗
    ({Θ;Δ;Γ} ⊨ e2 ≤log≤ e2' ≤log≤ e2'' : t_nat) -∗
    ({Θ;Δ;Γ} ⊨ e3 ≤log≤ e3' ≤log≤ e3'' : t_string) -∗
  ...
  Lemma bin_log_related_nat_unop Θ Δ Γ op e e' e'' τ :
  ...
  Lemma bin_log_related_bool_unop Θ Δ Γ op e e' e'' τ :
  ...
  Lemma bin_log_related_string_unop Θ Δ Γ op e e' e'' τ :
  ...
  Lemma bin_log_related_unfold Θ Δ Γ e e' e'' κ (τ : typ κ (Θ ▹ κ%kind)) (T : telim_ctx Θ κ ⋆) :
  ...
  Lemma bin_log_related_fold Θ Δ Γ e e' e'' κ (τ : typ κ (Θ ▹ κ%kind)) (T : telim_ctx Θ κ ⋆) :
  ...
  Lemma bin_log_related_pack' Θ κ Δ Γ e e' e'' τ τ' :
    ({Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ.[τ'/]) -∗
    {Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : ∃: κ, τ.
  Proof.
    iIntros "IH".
  ...
  Lemma bin_log_related_pack κ (A : kindO Σ κ) Θ Δ Γ e e' e'' τ :
    ({Θ ▹ κ; ext Δ A; ⤉ Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ) -∗
    {Θ;Δ;Γ} ⊨ e ≤log≤ e' ≤log≤e'' : ∃: κ, τ.
  Proof.
    iIntros "IH".
  ...
  Lemma bin_log_related_unpack Θ κ Δ Γ x e1 e1' e2 e2' e1'' e2'' τ τ2 :
  ...
  Proof.
    ...
    intro_clause'.
    pures.
    rel_bind_ap e1 e1' e1'' "IH1" v v' v'' "(? & ? & [%A #IH1])"; rewrite -/interp.
  ...
  Lemma bin_log_related_hash Θ Δ Γ e e' e'' :
    ({Θ; Δ; Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : t_string) -∗
    {Θ; Δ; Γ} ⊨ Hash e ≤log≤ Hash e' ≤log≤ Hash e'' : t_string.
  Proof.
    iIntros "IH".
    intro_clause'.
    rel_bind_ap e e' e'' "IH" v v' v'' "([#Hctx Hp] & [_ Hi] & (%&%&%&%))".
    simplify_eq.
    iMod (step_verifier_hash with "[$]") as "Hv"; [done|].
    iMod (step_ideal_hash with "[$]") as "Hi"; [done|].
    wp_apply (wp_hash with "[$]").
    iIntros "_". iFrame. eauto.
  Qed.

  Lemma bin_log_related_tequiv Θ Δ Γ e e' e'' τ τ' :
    Θ ⊢ₑ τ ≃ τ' : ⋆ →
    ({Θ; Δ; Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ) -∗
    ({Θ; Δ; Γ} ⊨ e ≤log≤ e' ≤log≤ e'' : τ').
  Proof.
