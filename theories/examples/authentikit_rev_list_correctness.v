...
From auth.examples Require Export authentikit_rev_list authentikit_base_correctness.
...
Section proof.
  ...
  Definition lrel_auth_comp' (A : lrel Σ) : lrel Σ := LRel (λ v1 v2 v3,
    ∀ t2 K2 t3 K3 (p : proph_id) (ps ps' : list string) (w w' : val),
      {{{ spec_verifier t2 (fill K2 (v2 w')) ∗
          ...
          ⌜is_proof w ps⌝ ∗ is_proph_proof w' p ps'
      }}}
        v1 #p w
  ...
  Proof.
    ...
    wp_apply ("HmA" with "[$Hv $Hi $Hw $Hw']").
    iIntros (ps1 ps2 u1 u1' a1 a2 a3) "(%Heq1 & Hu1 & Hu2 & Hv & Hi & #HA) /=".
    wp_pures. v_pures.
    ...
    wp_apply ("HmB" with "[$Hv $Hi $Hu1 $Hu2]").
    iIntros (ps3 ps4 x1 x2 ???) "(%Heq2 & Hx1 & Hx2 & Hv & Hi & #HB)".
  ...
  Proof.
    ...
    iIntros (????) "Hv Hi".
    v_pures; i_pures; wp_pures.
    iDestruct "Hauth" as (tA' a1 a2 s -> Hs2 ->) "(_ & #HA)".
