...
Definition suspSet (N : namespace) : namespace := N .@ "susp".
Definition suspFill (N : namespace) (ps : proph_id) : namespace := (suspSet N) .@ ps.
...
Section authenticatable.
  ...
  Definition proph_susp (p : proph_id) (b : bool) : iProp Σ :=
    (typed_proph1_prop BoolTypedProph) p b.
  ...
  Definition auth_is_ser_p_unsusp (v : val) (h : string) : iProp Σ :=
    ∃ (a : val) (s : string), 
      ⌜v = (a, #(hash s))%V ∧ h = (hash s)⌝ ∗ hashed s ∗
      s_is_ser (g:=gwp_upto_bad) auth_scheme a s.

  Definition auth_is_ser_p_susp_fill (v : val) (h : string) : iProp Σ :=
    ∃ (b r : bool) (lb lr : loc) (ps : proph_id) (a : val) (s : string),
      ⌜v = (#ps, #lb, #lr, a, #(hash s))%V⌝ ∗ hashed s ∗
      s_is_ser (g:=gwp_upto_bad) auth_scheme a s ∗
      seq_inv lb ↦ #b ∗ lb ↦ #r ∗ proph_susp ps false ∗
      ⌜(b = true ∧ r = true) ∨ (b = false)⌝.

  Definition auth_is_ser_p_susp_unfill (v : val) : iProp Σ :=
    ∃ (r : bool) (lb lr : loc) (ps : proph_id) (a : val) (s : string),
      ⌜v = (#ps, #lb, #lr, a, #(hash s))%V⌝ ∗ hashed s ∗
      lb ↦ #true ∗ lb ↦ #r ∗ proph_susp ps true ∗
      s_is_ser (g:=gwp_upto_bad) auth_scheme a s.
  ...
  Fixpoint s_is_ser_proph (t : evi_type) (v : val) (s : string) : iProp Σ :=
    ...
    | tint => int_is_ser v s
    | tauth => ∃ v1, ⌜v = SOMEV v1⌝ ∗ 
                ((auth_is_ser_p_unsusp v1 s ∨ auth_is_ser_p_susp_fill v1 s) ∨
                  (⌜s = suspended_string⌝ ∗ auth_is_ser_p_susp_unfill v1))
    end.
