...
Section finish_specs.
  ...
  Local Lemma vmauth_pn_eq vm mp a b ctr (Heq : a = b) :
    visited_mapg_auth vm mp a ctr -∗ visited_mapg_auth vm mp b ctr.
  ...
  Proof.
   iIntros (?? ????) "#Htab Hi".
    ...
    - v_pures. v_bind (ser _).
      ...
      iMod ("Hclose_tab" with "[$Htabtok Hbigsep Hl Hvmauth Hvisinv Hst' Hserp]") as "Htabtok".
      { iNext. iLeft. iFrame "% ∗". }
        (* iLeft. iFrame.
    ...
    - v_pures. v_bind (ser _).
      ...
      + iDestruct "Hinv_2" as "(%&%&%& Hlbfrag' & #Hvisfin & Hintr & (%&%&%&%&%&%&%&%& Hcap & Hunfill & Hmfrag & %Hmsub & %Hssf & #Hserpred_emp & Hsusp))".
        ...
        rewrite /filled_string /simple_string in H2a.
        apply (inj some_ser_str), (inj string_ser_str) in H2a as <-.
        iPoseProof (lg_mapg_agree with "Hlbfrag' Hlbfrag") as "(-> & _ & _)".
        ...
        * v_load. v_bind (map_remove _ _).
          ...
          { apply dom_insert_lookup_L.
            apply elem_of_dom. eapply dom_omap_subseteq.
            apply elem_of_dom. by eapply mk_is_Some. }
          ...
          iAssert (
            |={⊤}=> ∃ E',
              ...
              ((∃ γ vm',
                  ...
                  visited_mapg_auth vm' (<[pid:=csum.Cinr (to_agree ())]> m') pn idctr) ∨
               (vm_big_sep (delete #pid m) vm ∗
                visited_mapg_auth vm (<[pid:=csum.Cinr (to_agree ())]> m') pn idctr)) ∗
              [∗ map] k↦y0 ∈ mapg_alive (<[pid:=csum.Cinr (to_agree ())]> m'), 
                    v_susp_big_sep_lam m k y0)%I
              with "[Htok Hintr Hxauth Hvisinv Hvmauth Hbigsep]"
            as ">(%&->& #Hnmspc' & Hxauth & Hvisvm & Hbigsep)".
          { iDestruct "Hxauth" as "[[-> Hidtok]|
            ...
            -
              ...
              + iDestruct "Hinv_2" as "(%&%&%&%&%&%&%&%Hxpure& Hxcap & Hxunfill & Hxmfrag & %Hxmsub & %Hxssf & #Hxserpred_emp & Hxsusp)".
                ...
                iDestruct ("Hvmclose" with "[//] Hmauth") as "Hvmauth".
                destruct Hxin as [(?&Hxin&Heqx_pv0)%Some_equiv_eq Heqx_yx2].
                ...
                iDestruct "Hms" as (ctr ????????[Hxcgt [Hxin' Heqx_x2]])
                    "(Hlc & Hxxser & #Hxxserpred_big & Hxxserspec & Hxxauth & Hxxc & Hxxfin)".

                iMod (visit_update_finished with "Hvmauth Hxvisdone Hintr Hxlbfrag
                  Hxsusp Hxxc") as "(#Hxvisfin & Hintr & Hvmauth & Hxxc & Hxsusp)".
                { assert (x2 = pv0) as ->.
                  { rewrite Hyyx' in Heqx_x2. rewrite Heqx_x2 in Heqx_yx2. simpl in Heqx_yx2.
                    fold_leibniz. by apply (inj to_agree) in Heqx_yx2. }
                  exact Hxmsub. }
          ...
          iMod ("Hclose_tab" with "[$Htabtok Hl Hbigsep Hvisvm Hst' Hserp]") as "Htabtok".
          { iNext. iLeft. iFrame "% ∗".
            iDestruct "Hvisvm" as "[(%&%&-> & $ & $)|[$ $]]";
  ...
  Lemma flush_buf_stream_spec :
    ∀ (p : proph_id) (prf buf : val) (lpn : list nat) (pn : nat)
      ...
      {{{ prf' (ps' ps_proph' x : list string), RET prf';
          ...
          (∀ vm mp ctr,
            tern_state -∗ visited_mapg_auth vm mp pn ctr -∗
            pencount_frag pn ==∗
            tern_state ∗ visited_mapg_auth vm mp 0 ctr ∗
            pencount_frag 0) }}}.
  Proof.
    iIntros (?????????? ?) 
    ...
    - wp_apply gwp_list_head; try done.
      ...
      iMod ("Hgoodtr2" $! vm mp (pn + sum_list lpn) ctr with "Hgood Hpset Hpc Hvm [//] HbigS")
      ...
      iAssert (visited_mapg_auth vm mp (sum_list lpn) ctr) with "[Hvm_rem]" as "Hvm_rem".
      { iApply (vmauth_pn_eq vm _ _ _ ctr Hsumeq with "Hvm_rem"). }
