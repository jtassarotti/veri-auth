  ...
  Lemma flush_buf_stream_spec :
    ∀ (p : proph_id) (prf buf : val) (lpn : list nat) (pn : nat)
      ...
      {{{ prf' (ps' ps_proph' x : list string), RET prf';
          ...
          (∀ vm d pset ctr gm,
            good_state -∗ visited_mapg_auth vm d pset pn ctr gm -∗
            pencount_frag pn ==∗
            good_state ∗ visited_mapg_auth vm d ∅ 0 ctr gm ∗
            pencount_frag 0) }}}.
  Proof.
    iIntros (?????????? ?) 
    ...
    - wp_apply gwp_list_head; try done.
      ...
      iMod ("Hgoodtr" $! vm d pset (size γl + sum_list lpn) ctr gm with "Hgood Hpset Hpc Hvm [//] HbigS")
      ...
      iMod ("Hgoodtr'" $! vm d (pset ∖ γl) with "Hgood Hvm_rem Hpc") as "($ & $ & $)".
      done.
  Qed.
  ...
  Definition lrel_auth_comp_tern (A : lrel_tern Σ) : lrel Σ := LRel (λ v1 v2 v3,
    ∀ t2 K2 t3 K3 p (ps ps1 ps2 ps_fix : list string) (lpn : list nat) (w1 w2 : val),
      {{{ seq_tok ⊤ ∗ spec_verifier t2 (fill K2 (v2 w2)) ∗
          ...
          intransit 1%Qp ∗ good_state ∗ stok_unset
      }}}
        v1 w1
