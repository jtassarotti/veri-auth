  ...
  Proof.
    ...
    - iMod (na_inv_acc with "Hinv_unfill Htok") as "(>Hinvo & Htok & Hclose_inv)"; try solve_ndisj.
      ...
      + iDestruct "Hbrproph" as "(Hbrproph & %&%)".
        ...
        (* Mirror of unauth_step.v:222 — extract [mapg_auth m''] via
           visited_mapg_acc, return closer wand so Hvmauth can be
           reassembled if needed later. *)
        iAssert (|==> ∃ m'', mapg_auth m'' ∗
          ...
          last iMod "Hmint" as (m'') "(Hmauth & % & Hvmclose)".
        ...
        iAssert (
          ...
          with "[Htok Hintr Hidtok Hvmclose Hc Hmauth Hbigsep Hlb Hlr Hbrproph Hclose_inv]"
        as "Hbig_assert"; last
          iMod "Hbig_assert"
        ...
        -- assert (size γl > 0) as Hγlpos.
          ...
          (* Mono first, then insert (see unauth_step.v:418): insert's
             per-entry obligation needs the NEW m's lookup at #cntr, which
             only holds after the mono. *)
          iPoseProof (big_sepM_mono
              (v_susp_big_sep_lam m)
              (v_susp_big_sep_lam (<[#cntr:=(#(size γl), v_finish)%V]> m))
            with "Hbigsep") as "Hbigsep".
