...
Section visited_map_res.
  ...
  (** [visited_mapg_auth m mp pn ctr] bundles the entire visited-map authority
  ...
  Definition visited_map_update_pending
      (m : state_mapg_type) (mp : mapg_type) (γs : gset gname) (pn : nat) (ctr : nat) : iProp Σ :=
    visited_mapg_auth (set_fold (λ γ m, <[ γ := pending_val ]>m) m γs)
                      mp (pn + size γs) ctr.

  Definition visited_map_update_done
      (m : state_mapg_type) (mp : mapg_type) (γ : gname) (pn : nat) (ctr : nat) : iProp Σ :=
    ∃ n, visited_mapg_auth (<[ γ := done_val n ]>m) mp pn ctr.

  Definition visited_map_update_finished
      (m : state_mapg_type) (mp : mapg_type) (γ : gname) (pn : nat) (ctr : nat) : iProp Σ :=
    visited_mapg_auth (<[ γ := finished_val ]>m) mp pn ctr.

  Definition visited_mapg_pending_removed
      (m : state_mapg_type) (mp : mapg_type) (γs : gset gname) (pn : nat) (ctr : nat) : iProp Σ :=
    visited_mapg_auth m mp (pn - size γs) ctr.

  Lemma visited_map_update_pending_rewrite m mp γs pn ctr :
    visited_map_update_pending m mp γs pn ctr ⊣⊢
    visited_mapg_auth (set_fold (λ γ m, <[ γ := pending_val ]>m) m γs) mp (pn + size γs) ctr.
  Proof. rewrite /visited_map_update_pending. done. Qed.

  Lemma visited_map_update_done_rewrite m mp γ pn ctr :
    visited_map_update_done m mp γ pn ctr ⊣⊢
    ∃ n, visited_mapg_auth (<[ γ := done_val n ]>m) mp pn ctr.
  Proof. rewrite /visited_map_update_done. done. Qed.

  Lemma visited_map_update_finished_rewrite m mp γ pn ctr :
    visited_map_update_finished m mp γ pn ctr ⊣⊢
    visited_mapg_auth (<[ γ := finished_val ]>m) mp pn ctr.
  Proof. rewrite /visited_map_update_finished. done. Qed.

  Lemma visited_mapg_pending_remove_rewrite m mp γs pn ctr :
    visited_mapg_pending_removed m mp γs pn ctr ⊣⊢
    visited_mapg_auth m mp (pn - size γs) ctr.
  ...
  Lemma id_ctr_frag_agree m mp pn ctr ctr' :
  ...
  Lemma visited_insert m mp pn ctr :
    visited_mapg_auth m mp pn ctr ∗ pencount_frag pn ==∗
      ∃ γ,
  ...
  (* visited_transition_done has been subsumed by [bind_id_existing_susp] —
     binding now writes to gm and m_v atomically to maintain
     [id_susp_gamma_coherent]. *)

  Lemma visited_transition_finished m mp pn ctr γ n :
  ...
  Lemma visit_done_lookup γ n m mp pn ctr :
    visited_mapg_auth m mp pn ctr -∗ visit_done γ n -∗
      ⌜m !! γ = Some (done_val n)⌝ ∗
      visited_mapg_auth m mp pn ctr ∗ visit_done γ n.
  Proof.
  ...
  Lemma visit_finished_lookup γ m mp pn ctr :
  ...
  Lemma visit_reached_done_lookup γ m mp pn ctr :
    visited_mapg_auth m mp pn ctr -∗ visit_reached_done γ -∗
      ⌜(∃ n, m !! γ = Some (done_val n)) ∨ m !! γ = Some finished_val⌝ ∗
  ...
  Lemma pending_set_remove m mp pn ctr γs :
    visited_mapg_auth m mp pn ctr -∗ pencount_frag pn -∗ penset_frag γs -∗
  ...
  Lemma id_token_unused m mp pn ctr id :
  ...
  (** [id_ctr_frag_alloc] allocates the next fresh id-token paired with
      ...
      entries (and the γ binding) are written later by [bind_id_existing_susp].

      Returns the pure fact [⌜rs !! ctr = None⌝] so that callers can chain
      directly into the binding step (whose [rs !! from = None]
  ...
  Lemma id_ctr_frag_alloc m mp pn ctr (l : loc) :
    visited_mapg_auth m mp pn ctr -∗ id_ctr_frag ctr ==∗
  ...
  (** Mint a persistent [pval_frag id susp ∗ (lg_mapg_v_frag susp γ)] witness
      ...
      in [Section lg_map]. *)
  Lemma id_susp_γ_lookup m mp pn ctr γ id :
    m !! γ = Some (done_val id) →
    visited_mapg_auth m mp pn ctr ==∗
      visited_mapg_auth m mp pn ctr ∗
...
Section lg_map.
  ...
  (* Prover-side fragments. The fragment carries the per-loc
     [meta l lg_p_meta_n true] mark set at insertion, so that the
     never-registered mark below excludes it WITHOUT the authority. *)
  Definition lg_mapg_p_frag (l : loc) (γ : gname) : iProp Σ :=
    own lg_mapG_p_name (◯ ({[ l := Cinr (to_agree γ) ]} : gmap loc lg_mapEntry)) ∗
    meta l lg_p_meta_n (true : bool).

  (* Persistent never-registered mark: mintable LOCALLY from the
     allocation's [meta_token] (no [lg_p_auth] needed — this is what the
     unary evidence world relies on). [lg_p_insert]'s [meta_combine_dom]
     sets [true] at the same namespace, so a marked loc can never be
     registered and vice versa. *)
  Definition lg_mapg_p_unalloc (l : loc) : iProp Σ :=
    meta l lg_p_meta_n (false : bool).
  ...
  Proof.
    rewrite /lg_mapg_p_frag. iIntros "[H1 #Hm1] [H2 #Hm2]".
    iDestruct (own_valid_2 with "H1 H2") as %Hv.
    iFrame "H1 H2 Hm1". iPureIntro.
    rewrite -auth_frag_op auth_frag_valid singleton_op singleton_valid -Cinr_op
      Cinr_valid in Hv.
    fold_leibniz. by apply to_agree_op_inv_L in Hv.
  Qed.
  ...
  Proof.
    rewrite /lg_mapg_p_frag /lg_mapg_p_unalloc. iIntros "[_ H1] H2".
    by iDestruct (meta_agree with "H1 H2") as %?.
  Qed.

  (** Mint the never-registered mark from a fresh allocation's
      [meta_token] — auth-free, hence available in the unary world. *)
  Lemma lg_p_unalloc_mint (l : loc) :
    meta_token l ⊤ ==∗ lg_mapg_p_unalloc l.
  Proof.
    rewrite /lg_mapg_p_unalloc.
    iIntros "Hmtok".
    iDestruct (meta_token_difference l (↑lg_p_meta_n) with "Hmtok")
      as "[Hmtok_N _]"; first set_solver.
    by iMod (meta_set _ l (false : bool) lg_p_meta_n with "Hmtok_N").
  Qed.
  ...
  Local Lemma meta_combine_dom (m : lg_mapg_type) (l : loc) (e : lg_mapEntry) :
    meta_token l ⊤ -∗ ([∗ set] l' ∈ dom m, meta l' lg_p_meta_n (true : bool)) ==∗
      ⌜l ∉ dom m⌝ ∗ meta l lg_p_meta_n (true : bool) ∗
  ...
  Proof.
    ...
    iMod (meta_combine_dom m_p l (Cinr (to_agree γ)) with "Hmtok Hbig_p")
      as "(%Hl_nin & #Hmeta & Hbig_p')".
    apply not_elem_of_dom in Hl_nin.
    iMod (own_update with "Hauth_p") as "[$ Hfrag]".
    { apply auth_update_alloc. by apply alloc_singleton_local_update. }
    iModIntro. iFrame "Hauth_v Hsmeta Hbig_p' Hfrag Hmeta".
  Qed.
  ...
  Proof.
    ...
    iMod (own_update with "Hauth_p") as "[$ Hfrag]".
    { apply auth_update_alloc. by apply alloc_singleton_local_update. }
    iModIntro. iFrame "Hbig_p' Hfrag Hmeta".
  Qed.
  ...
  (** [bind_id_existing_susp]: bind an existing id [from] (whose susp loc
      ...
      bound to. Unlike a fresh-susp binding, it does not write
  ...
  Lemma bind_id_existing_susp
    ...
    visited_mapg_auth m mp pn ctr ==∗
      visited_mapg_auth (<[γ := done_val from]>m) mp pn ctr ∗
      visit_done γ from ∗
      id_ref_frag from to.
  Proof.
  ...
  Lemma pval_snapshot_alloc m mp pn ctr susp pid (M : gmap nat loc) :
    dom M = set_seq 0 (S pid) →
    vmeta_token susp -∗
    visited_mapg_auth m mp pn ctr -∗
  ...
  (** [pval_snapshot susp pid] + [pval_frag id' susp'] with [id' ≤ pid]
  ...
  Lemma pval_snapshot_neq susp pid id' susp' :
    id' ≤ pid →
    pval_snapshot susp pid -∗ pval_frag id' susp' -∗ ⌜susp ≠ susp'⌝.
  Proof.
    iIntros (Hlt) "(%M & %Hdom & %Hnotin & #Hsnap) #Hpv".
