...
From auth.examples Require Export authentikit_base_correctness authentikit_buf_magic .
...
Section proof.
  ...
  Definition is_proof_state (p : proph_id) (v1 v2 : val) (ps ps' : list string) : iProp Σ :=
    ∃ (prf1 cache1 prf2 cache2 : val) (m : gmap string val),
  ...
  Proof.
    ...
    - apply elem_of_dom in Hb as [a Ha].
      ...
      wp_pures.
      v_pures.
      iDestruct (big_sepM_lookup with "Hhash") as (s' t Hs' ?) "#Hs'"; [done|].
    ...
    - rewrite /p_add_obj. wp_pures.
      ...
      iMod (gwp_map_insert (K := string) (V := val) _ _ _ _ () ⊤
                  _ (λ v, ⌜is_map v (<[hash _:=a2]> m)⌝)%I
                 with "[//] [] [$Hv //]") as (x) "[Hv %] /=".
      { by iIntros (?). }
      Unshelve. 2 : { done. }
  ...
  Proof.
    ...
    iMod (gwp_map_empty string val _ () ⊤ (λ v, ⌜is_map v (∅ : gmap string val)⌝)%I
           with "[% //] [] [$Hv //]") as (x) "[Hv %Hx] /=".
    { by iIntros "!#" (??). }
    v_pures.
