...
From auth.examples Require Export authentikit_buf_magic authentikit_base_security.
...
Section proof.
  ...
  Proof.
    ...
    destruct (Hc !! hash s1) as [a|] eqn:Hlookup; iIntros (? ->).
    - iDestruct (big_sepM_lookup with "Hcache") as (s1' t ??) "Hs1'"; [done|subst].
      wp_pures.
    ...
    - wp_pures.
      ...
      iExists (Some _). iModIntro. iSplit; [done|]. iFrame.
  ...
  Proof.
    ...
    iIntros (f1) "(%f2 & Hi & Hc) /=".
    wp_pures; i_pures.
    wp_apply (gwp_map_empty string val); [done|].
    iIntros (??).
    iSpecialize ("Hc" with "[] Hi").
