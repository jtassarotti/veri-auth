...
(** This file contains the adequacy statements of the up-to-bad program logic. First
we prove a number of auxilary results. *)

Definition step_not_bad {Λ} (bad_state : state Λ → Prop) ρ1 κ ρ2 :=
  step ρ1 κ ρ2 ∧ ¬ bad_state ρ2.2.

Inductive nsteps_not_bad {Λ} (bad_state : state Λ → Prop) :
  nat → cfg Λ → list (observation Λ) → cfg Λ → Prop :=
| nsteps_not_bad_refl ρ :
  nsteps_not_bad bad_state 0 ρ [] ρ
| nsteps_not_bad_l n ρ1 ρ2 ρ3 κ κs :
  step_not_bad bad_state ρ1 κ ρ2 →
  nsteps_not_bad bad_state n ρ2 κs ρ3 →
  nsteps_not_bad bad_state (S n) ρ1 (κ ++ κs) ρ3.
...
Notation step_not_bad := (step_not_bad bad_state).
Notation nsteps_not_bad := (nsteps_not_bad bad_state).
Notation wptp s t Φs := ([∗ list] e;Φ ∈ t;Φs, WP e @ s; ⊤ {{ Φ }})%I.

Local Lemma wp_step s e1 σ1 ns κ κs e2 σ2 efs nt Φ :
  prim_step_not_bad e1 σ1 κ e2 σ2 efs →
...
Local Lemma wptp_preservation s n es1 es2 κs κs' σ1 ns σ2 Φs nt :
  nsteps_not_bad n (es1, σ1) κs (es2, σ2) →
...
Local Lemma wp_progress_gen (hlc : has_lc) Σ Λ `{!invGpreS Σ} es σ1 n κs t2 σ2 e2
        (num_laters_per_step : nat → nat) (bad_state : state Λ → Prop):
    ...
    ⊢ |={⊤}=> ∃
       ...
       let _ : irisGS_gen hlc Λ Σ := IrisG Hinv stateI fork_post num_laters_per_step
                                  state_interp_mono bad_state
       in
       stateI σ1 0 κs 0 ∗
       ([∗ list] e;Φ ∈ es;Φs, WP e @ ⊤ {{ Φ }})) →
  nsteps_not_bad bad_state n (es, σ1) κs (t2, σ2) →
  e2 ∈ t2 →
  not_stuck e2 σ2.
Proof.
  intros Hwp ??.
  ...
  iMod (@wptp_progress _ _ _
       (IrisG Hinv stateI fork_post num_laters_per_step state_interp_mono bad_state) _ []
...
Definition erased_step_not_bad {Λ} (bad_state : state Λ → Prop) (ρ1 ρ2 : cfg Λ) :=
  ∃ κ, step_not_bad bad_state ρ1 κ ρ2.
...
Record adequate {Λ} (bad_state : state Λ → Prop) (s : stuckness) (e1 : expr Λ) (σ1 : state Λ)
    (φ : val Λ → state Λ → Prop) := {
  adequate_result t2 σ2 v2 :
   ¬ bad_state σ1 →
   rtc (erased_step_not_bad bad_state) ([e1], σ1) (of_val v2 :: t2, σ2) → φ v2 σ2;
  adequate_not_stuck t2 σ2 e2 :
   ¬ bad_state σ1 →
   s = NotStuck →
   rtc (erased_step_not_bad bad_state) ([e1], σ1) (t2, σ2) →
   e2 ∈ t2 →
   not_stuck e2 σ2
}.

Lemma adequate_alt {Λ} bad_state s e1 σ1 (φ : val Λ → state Λ → Prop) :
  ¬ bad_state σ1 →
  adequate bad_state s e1 σ1 φ ↔ ∀ t2 σ2,
    rtc (erased_step_not_bad bad_state) ([e1], σ1) (t2, σ2) →
...
Lemma adequate_trivial {Λ} bad_state s e1 σ1 (φ : val Λ → state Λ → Prop) :
  bad_state σ1 → adequate bad_state s e1 σ1 φ.
Proof. intros. constructor; naive_solver. Qed.

Theorem adequate_tp_safe {Λ} bad_state (e1 : expr Λ) t2 σ1 σ2 φ :
  ¬ bad_state σ1 →
  adequate bad_state NotStuck e1 σ1 φ →
  rtc (erased_step_not_bad bad_state) ([e1], σ1) (t2, σ2) →
...
Proof.
  ...
  destruct (adequate_not_stuck bad_state NotStuck e1 σ1 φ Had t2 σ2 e2) as [?| (κ&e3&σ3&efs&?)];
    rewrite ?eq_None_not_Some; auto.
  { exfalso. eauto. }
  destruct (elem_of_list_split t2 e2) as (t2'&t2''&->); auto.
  right; exists (t2' ++ e3 :: t2'' ++ efs), σ3, κ.
  econstructor;eauto.
Qed.

Lemma erased_steps_nsteps_not_bad {Λ} bad_state (ρ1 ρ2 : cfg Λ) :
  rtc (erased_step_not_bad bad_state) ρ1 ρ2 ↔ ∃ n κs, nsteps_not_bad bad_state n ρ1 κs ρ2.
Proof.
  split.
  - induction 1.
    + do 2 eexists. by econstructor.
    + destruct H. destruct IHrtc as (?&?&?).
      do 2 eexists. by econstructor.
  - intros (n & κs & Hsteps). unfold erased_step_not_bad.
...
Lemma wp_adequacy_gen (hlc : has_lc) Σ Λ `{!invGpreS Σ} s e σ φ bad_state `{∀ σ, Decision (bad_state σ)} :
     ...
     ⊢ |={⊤}=> ∃
       ...
       let _ : irisGS_gen hlc Λ Σ :=
           IrisG Hinv (λ σ _ κs _, stateI σ κs) fork_post (λ _, 0)
                 (λ _ _ _ _, fupd_intro _ _) bad_state
       in
       stateI σ κs ∗ WP e @ s; ⊤ {{ v, ⌜φ v⌝ }}) →
  adequate bad_state s e σ (λ v _, φ v).
Proof.
  intros Hwp.
  destruct (decide (bad_state σ)) as [|Hnb].
  { by apply adequate_trivial. }
  apply adequate_alt; [done|]; intros t2 σ2 [n [κs ?]]%erased_steps_nsteps_not_bad.
...
Lemma wp_invariance_gen (hlc : has_lc) Σ Λ `{!invGpreS Σ} s e1 σ1 t2 σ2 φ bad_state :
  (∀ `{Hinv : !invGS_gen hlc Σ} κs,
     ⊢ |={⊤}=> ∃
         (stateI : state Λ → list (observation Λ) → nat → iProp Σ)
         (fork_post : val Λ → iProp Σ),
       let _ : irisGS_gen hlc Λ Σ := IrisG Hinv (λ σ _, stateI σ) fork_post
              (λ _, 0) (λ _ _ _ _, fupd_intro _ _) bad_state in
       stateI σ1 κs 0 ∗ WP e1 @ s; ⊤ {{ _, True }} ∗
       (stateI σ2 [] (pred (length t2)) -∗ ∃ E, |={⊤,E}=> ⌜φ⌝)) →
  rtc (erased_step_not_bad bad_state) ([e1], σ1) (t2, σ2) →
  φ.
Proof.
  intros Hwp [n [κs ?]]%erased_steps_nsteps_not_bad.
