...
Class irisGS_gen (hlc : has_lc) (Λ : language) (Σ : gFunctors) := IrisG {
  ...
  (** Characterization of "bad" states that we do not consider *)
  bad_state : state Λ → Prop;
}.
...
Definition prim_step_not_bad `{!irisGS_gen hlc Λ Σ} e1 σ1 κ e2 σ2 efs :=
  prim_step e1 σ1 κ e2 σ2 efs ∧ ¬ bad_state σ2.
...
Definition wp_pre `{!irisGS_gen hlc Λ Σ} (s : stuckness)
  ...
  | None => ∀ σ1 ns κ κs nt,
     state_interp σ1 ns (κ ++ κs) nt ={E,∅}=∗
       ...
       ∀ e2 σ2 efs, ⌜prim_step_not_bad e1 σ1 κ e2 σ2 efs⌝ -∗
...
Proof.
  ...
  iModIntro. iIntros (e2 σ2 efs [Hstep Hbad]) "Hcred".
...
Proof.
  ...
  iIntros (e2 σ2 efs [Hstep Hbad]) "Hcred".
...
Proof.
  ...
  iIntros (e2 σ2 efs [Hstep Hbad]) "Hcred".
  iMod ("H" $! _ _ _ with "[] Hcred") as "H".
  { iSplit; eauto using fill_step. }
