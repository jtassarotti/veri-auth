...
From auth.program_logic_upto_bad Require Export total_weakestpre.
...
Lemma twp_lift_step s E Φ e1 :
  ...
  (∀ σ1 ns κs nt, state_interp σ1 ns κs nt ={E,∅}=∗
    ...
    ∀ κ e2 σ2 efs, ⌜prim_step_not_bad e1 σ1 κ e2 σ2 efs⌝ ={∅,E}=∗
      ⌜κ = []⌝ ∗
...
Lemma twp_lift_pure_step_no_fork `{!Inhabited (state Λ)} s E Φ e1 :
  ...
  (∀ σ1 κ e2 σ2 efs, prim_step_not_bad e1 σ1 κ e2 σ2 efs → κ = [] ∧ σ2 = σ1 ∧ efs = []) →
  (|={E}=> ∀ κ e2 efs σ, ⌜prim_step_not_bad e1 σ κ e2 σ efs⌝ → WP e2 @ s; E [{ Φ }])
  ⊢ WP e1 @ s; E [{ Φ }].
Proof.
...
Lemma twp_lift_pure_det_step_no_fork `{!Inhabited (state Λ)} {s E Φ} e1 e2 :
  (∀ σ1, reducible_no_obs e1 σ1) →
  (∀ σ1 κ e2' σ2 efs', prim_step_not_bad e1 σ1 κ e2' σ2 efs' →
    κ = [] ∧ σ2 = σ1 ∧ e2' = e2 ∧ efs' = []) →
...
Proof.
  ...
  iApply twp_lift_pure_det_step_no_fork; [done| |].
  { intros ????? []. auto. }
