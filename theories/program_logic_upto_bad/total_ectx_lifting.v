...
From auth.program_logic_upto_bad Require Export total_weakestpre total_lifting.
...
Lemma twp_lift_base_step_not_bad {s E Φ} e1 :
  to_val e1 = None →
  (∀ σ1 ns κs nt, state_interp σ1 ns κs nt ={E,∅}=∗
    ⌜base_reducible_no_obs e1 σ1⌝ ∗
    ∀ κ e2 σ2 efs, ⌜base_step e1 σ1 κ e2 σ2 efs ∧ ¬ bad_state σ2⌝ ={∅,E}=∗
      ⌜κ = []⌝ ∗
      state_interp σ2 (S ns) κs (length efs + nt) ∗
      WP e2 @ s; E [{ Φ }] ∗
      [∗ list] i ↦ ef ∈ efs, WP ef @ s; ⊤ [{ fork_post }])
...
Lemma twp_lift_atomic_base_step_not_bad {s E Φ} e1 :
  to_val e1 = None →
  (∀ σ1 ns κs nt, state_interp σ1 ns κs nt ={E}=∗
    ⌜base_reducible_no_obs e1 σ1⌝ ∗
    ∀ κ e2 σ2 efs, ⌜base_step e1 σ1 κ e2 σ2 efs ∧ ¬ bad_state σ2⌝ ={E}=∗
