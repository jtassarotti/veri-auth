(* This file implements the merkle functor that uses authentikit.
   It also implements a hand-optimized merkle retrieve operations and
   ties them with the merkle functor that uses authentikit. *)
        ...
        | Some (retrieve_proof_ser, proof_tail) ->
          ...
          | Some r_proof ->
            ...
            | Some proof_list ->
              ...
              in
              if imm_return then failwith "claimed shorter proof, but no path truncation" else
              match retrieve_magic_verifier_aux path proof_list start_hash with
              | None -> failwith "path proof mismatch" 
              | Some node_hash ->
                if node_hash = t_hash 
                then (proof_tail, result)
                else failwith "hash check failed" ) 
  end
