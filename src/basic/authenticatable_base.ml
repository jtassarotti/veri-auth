...
let pair_deserialize deser_a deser_b s =
  ...
  | Some i ->
    ...
    | None -> None
    | Some alen ->
      if string_of_int alen <> alen_str then None
      else if alen < 0 then None 
      else
...
let int_deserialize s = 
  if String.length s < 3 then None
    ...
    let rest = String.sub s 2 ((String.length s)-2) in
    if tag = "i_" then
      let mz = try Some (int_of_string rest) with Failure _ -> None in
      match mz with
      | None -> None
      | Some z -> if string_of_int z = rest then mz else None
    else None
